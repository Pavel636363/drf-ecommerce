...
from apps.shop.serializers import CategorySerializer, ProductSerializer, OrderItemSerializer, \
    ToggleCartItemSerializer, CheckoutSerializer, OrderSerializer, ReviewSerializer
from apps.sellers.models import Seller
from apps.profiles.models import OrderItem, ShippingAddress, Order
from apps.shop.filters import ProductFilter
from apps.shop.schema_example import PRODUCT_PARAM_EXAMPLE
...
class ProductReviewListCreateView(ListCreateAPIView):
    """
    GET: список отзывов по товару
    POST: создать отзыв к товару (один на пользователя)
    """
    serializer_class = ReviewSerializer
    permission_classes = [permissions.IsAuthenticatedOrReadOnly]

    @extend_schema(
        summary="Product Reviews List & Create",
        description="Список отзывов по товару и создание нового отзыва",
        tags=tags,
    )
    def get(self, request, *args, **kwargs):
    ...
    @extend_schema(
        ...
        description="Создать отзыв на товар (один отзыв на пользователя)",
        tags=tags,
    )
    def post(self, request, *args, **kwargs):
    ...
    def perform_create(self, serializer):
        ...
        product = Product.objects.get_or_none(slug=product_slug)
        if not product:
            # просто кинем ValidationError, DRF вернёт 400
...
class ProductReviewDetailView(RetrieveUpdateDestroyAPIView):
    ...
    (разрешить изменение/удаление только автору)
    """
    serializer_class = ReviewSerializer
    permission_classes = [permissions.IsAuthenticatedOrReadOnly]
    ...
    def perform_update(self, serializer):
        if self.get_object().user != self.request.user:
            raise serializers.ValidationError("Нельзя изменять чужой отзыв.")
        serializer.save()

    def perform_destroy(self, instance):
        if instance.user != self.request.user:
            raise serializers.ValidationError("Нельзя удалять чужой отзыв.")
        instance.delete()
